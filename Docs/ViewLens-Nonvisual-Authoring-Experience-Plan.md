...
- [x] **NV-0.4** Create deterministic fixture screens containing missing names, visible/programmatic-name conflicts, incorrect grouping, meaningful-order defects, modal focus loss, color-only state, clipped AX5 content, and a material visual regression. *(Implemented in [NonvisualFixtureSuite.swift](Sources/ViewLensKit/Nonvisual/NonvisualFixtureSuite.swift) and verified by unit tests.)*
